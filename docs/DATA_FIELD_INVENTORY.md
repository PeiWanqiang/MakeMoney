...
## 2. 源里有但当前丢弃（免费补，强烈建议第一优先）

Binance K 线归档/接口是 12 列，但 `parseBinanceKline`（`src/data/historical-dataset.ts:99`）只读 0–5 + 8。Spot 月包与 USD-M 月包**同一个 ZIP 里还有**：

| 列 | 字段 | 含义 | 当前 |
|---|---|---|---|
| 7 | `quote_asset_volume` | 成交额（quote 计价，**换手率的基础分母**） | ✗ 丢弃 |
| 8 | `number_of_trades` | 笔数 | ✓ 已存 |
| 9 | `taker_buy_base_volume` | 主动买量（base） | ✗ 丢弃 |
| 10 | `taker_buy_quote_volume` | 主动买成交额（quote） | ✗ 丢弃 |

**改造**（1 个可重跑不重下的动作）：
- `MarketBar` / `NormalizedMarketBar` 增加 `quoteVolume?`、`takerBuyBaseVolume?`、`takerBuyQuoteVolume?`。
- `parseBinanceKline` 补读 7/9/10 列；Parquet/snapshot schema 与 manifest 版本升级（新增字段为可空，旧分区仍可读）。
- Strategy SDK / 契约新增可引用字段，如 `market.quoteVolume`、`market.takerBuyRatio = takerBuyBase / volume`。
- 存量 ZIP 只需重跑解析（下载缓存仍在，`data/cache/`），不需要重新下载。

Web 侧 `parseKlineRows`（`web/worker/backtest-api.ts:781`）同样只读前 6 列；Phase 5 数据所有权并入服务后由服务统一承担，无需单独修。
