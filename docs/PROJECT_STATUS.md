...
项目已经从量化研究原型转回产品主路径：受约束 TypeScript 策略可以被完整类型检查、哈希并在 QuickJS/WASM 沙箱执行；自然语言策略 provider、结构化产物、编译错误自动修复、反向解释、不可变版本、源码 diff 和可选真实数据回测已经形成一个本地 CLI 纵切。
...
6. **AI 生成闭环**
   ...
   - [部分完成] 增加程序反向解释、源码 diff 和不可变修改版本；语义级机器校验和用户确认 UI 未完成。
   - 建立至少 100 条真实用户意图评测集。
