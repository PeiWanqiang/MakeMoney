...
最后更新：2026-07-31
当前阶段：Phase 1 Web 产品纵切 + 语义锁定参数优化实验室（Stage 1）
...
2026-07-31 开始“策略语义锁定下的参数优化实验室”新阶段。Stage 1 已完成参数槽位自动抽取、结构语义指纹、受限参数范围、确定性候选生成、同一数据快照批量回测、70/30 时间顺序训练/验证、Pareto 标记、实验缓存、持久化和 Web 对比界面。独立盲测、walk-forward、成本压力测试、参数邻域稳定性、候选正式版本化、异步任务队列和 AI 实验洞察仍未完成，因此当前推荐只算研究候选，不算正式策略或未来收益证据。
...
- [x] 70% 训练、30% 验证按时间顺序隔离；输出基准、推荐候选、Pareto 前沿、训练/验证指标和完整排名。
- [x] 未知参数、重复参数、非法范围、过多参数和越界取值会被服务端拒绝。
- [ ] Stage 2：walk-forward、最终盲测、相邻参数敏感性热图、手续费/滑点压力测试、多重选择偏差修正。
- [ ] Stage 3：AI 只读实验设计与结果解释；AI 不负责数值执行，也不能修改历史结果。
- [ ] 候选在完成 Stage 2 门禁前不能自动升级为正式策略版本。
...
- [部分完成] Web 产品：自然语言澄清、代码/解释视图、用户确认、回测报告、数据图表和 Stage 1 参数实验室已完成；登录和对话版本编辑未完成。
