...
- [x] `npm test`：10 个测试文件、35 项测试全部通过。
...
- [x] 对全部 14 条失败样例完成意图消歧、闭合 Bar 偏移提示、抽取器/场景器/比较器修复后，定向回归 14/14 通过；0 个契约错配、0 个生成错误、0 个 provider 错误。

说明：14/14 是失败集定向回归，不等同于新版全量 100/100；完整 V3 全量重跑仍需执行。评测语料仍是合成表达，不能替代真实用户语料。
...
- [x] 语义不一致也进入定点修复；无法覆盖的意图必须返回 `needs_clarification`，禁止偷偷用近似策略替代。
...
- [ ] 已有 100 条合成表达，但尚无真实用户自然语言语料评测集和 Web 交互界面。
...
6. **AI 生成闭环**
   ...
   - [已完成] 对 100 条合成意图运行真实 DeepSeek 生成评测并记录准确率、自动修复、延迟、token 和成本；首轮失败集已 14/14 定向回归通过。
   - [待完成] 完整重跑 V3 的 100 条合成意图，并收集、标注至少 100 条真实用户意图做独立评测。
...
> 收集首批 20 条中英文真实策略意图，对“生成成功率、自动修复率、语义保持、解释正确性、延迟和单次成本”做可重复评测；随后把已经验证的 CLI 闭环包进最小 Web 对话界面。

在真实用户语料到位前，可以先完整重跑 V3 的 100 条合成表达，作为工程回归基线；但不能把该结果当成产品准确率。
