...
- DeepSeek 真实批量评测支持内容指纹、断点续跑、并发、token/延迟/成本记录和离线语义重算；首轮 100 条通过 86 条，针对全部 14 条失败样例修复后定向回归 14/14 通过。
...
1. 完整重跑 V3 的 100 条合成表达；随后收集真实用户策略语料，用独立评测集验证语义保持、澄清率、修复率、延迟和成本。
